...
#     new_img = np.empty([128,1,28,28])
#     for i in range(x.shape[0]):
#         im = Image.fromarray(np.uint8(x[i].squeeze()),mode='L')
#         output = BytesIO()
#         im.save(output, format='PNG')
#         im = MotionImage(blob=output.getvalue())

#         im.motion_blur(radius=c[0], sigma=c[1], angle=np.random.uniform(-45, 45))

#         im = cv2.imdecode(np.frombuffer(im.make_blob(), np.uint8),
#                          cv2.IMREAD_UNCHANGED)
        
#         new_img[i,:,:,:] = np.clip(np.array(im), 0, 255).astype(np.float32)
